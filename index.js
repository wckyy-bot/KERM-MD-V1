...
async function connectToWA() {
  ...
  conn.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      let up = `*╭──────────────●●►*
> *➺Kᴇʀᴍ_ᴍᴅ-ᴠ1 ᴄᴏɴɴᴇᴄᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ ᴛʏᴘᴇ .ᴍᴇɴᴜ ᴛᴏ ᴄᴏᴍᴍᴀɴᴅ ʟɪsᴛ ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴋɢ ᴛᴇᴄʜ✅*
...
*╰──────────────●●►*`;
  ...
  conn.ev.on('messages.upsert', async(mek) => {
    ...
    const body = (type === 'conversation') ? mek.message.conversation : (type === 'extendedTextMessage') ? mek.message.extendedTextMessage.text : (type == 'imageMessage') && mek.message.imageMessage.imageMessage.caption : ''
    ...
    if (!isReact && senderNumber !== botNumber) {
      if (config.AUTO_REACT === 'true') {
        const reactions = ['😊', '👍', '😂', '💯', '🔥', '🙏', '🎉', '👏', '😎', '🤖', '👫', '👭', '👬', '👮', "🕴️", '💼', '📊', '📈', '📉', '📊', '📝' // Add more reactions if needed
    ...
    if (!isReact && senderNumber !== botNumber) {
      if (config.HEART_REACT === 'true') {
        const reactions = ['💘', '💝', '💖', '💗', '💓', '💞', '💕', '❣️', '❤️‍🔥', '❤️‍🩹', '❤️', '🩷', '🧡', '💛', '💚', '💙', '🩵', '💜' // Add more reactions if needed
        const randomReaction = reactions[Math.floor(Math.random() * reactions.length)]; // 
        m.react(randomReaction);
      }
    }
    ...
    events.commands.map(async(command) => {
      if (body && command.on === "body") {
        ...
        command.function(conn, mek, m,{from, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, isBotAdmins, isAdmins})
      } else if (
...
async function startSock() {
  const conn = makeWASocket({
    printQRInTerminal: true,  // Affiche le QR code dans le terminal pour la première connexion
    auth: state
  });
  ...
  // Exemple d'utilisation : quand un message est reçu, on log le contenu
  conn.ev.on('messages.upsert', m => {
    console.log(JSON.stringify(m, null, 2));
  });

  // Tu pourras maintenant utiliser conn.groupParticipantsUpdate pour bannir/débannir des utilisateurs
  // Exemple :
  // await conn.groupParticipantsUpdate('12345-67890@g.us', ['1234567890@s.whatsapp.net'], 'remove');
}
